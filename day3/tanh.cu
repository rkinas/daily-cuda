...
// CPU implementation of Tanh
void tanhCPU(const float* input, float* output, int n) {
    for (int i = 0; i < n; ++i) {
        output[i] = std::tanh(input[i]);
    }
}

// CUDA kernel for Tanh
__global__ void tanhCUDA(const float* input, float* output, int n) {
...
int main() {
    ...
    tanhCPU(h_input, h_output_cpu, N);

    // Run CUDA implementation
    tanhCUDA<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, N);
