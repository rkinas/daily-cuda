...
void sigmoidCPU(const float* input, float* output, int n) {
    for (int i = 0; i < n; ++i) {
...
__global__ void sigmoidCUDA(const float* input, float* output, int n) {
...
int main() {
    ...
    sigmoidCPU(h_input, h_output_cpu, N);

    // Run CUDA implementation
    sigmoidCUDA<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, N);
